...
model.save_weights('./checkpoints/con_score_model')
user_encoder = keras.Model(user_input, user_feat)
item_encoder = keras.Model(item_input, item_feat)
user_encoder.save_weights('./checkpoints/con_user_encoder')
item_encoder.save_weights('./checkpoints/con_item_encoder')

with open('con_user.npy', 'wb') as f:
    np.save(f, user_encoder(users).numpy())

with open('con_item.npy', 'wb') as f:
