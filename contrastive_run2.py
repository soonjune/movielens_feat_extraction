...
model.compile(loss=tf.nn.sigmoid_cross_entropy_with_logits, optimizer=tf.keras.optimizers.Adam(learning_rate=lr_schedule),
	metrics=["mse"])
...
model.save_weights('./checkpoints/con_score_model2')
user_encoder = keras.Model(user_input, user_feat)
item_encoder = keras.Model(item_input, item_feat)
user_encoder.save_weights('./checkpoints/con_user_encoder2')
item_encoder.save_weights('./checkpoints/con_item_encoder2')

with open('con_user2.npy', 'wb') as f:
    np.save(f, user_encoder(users).numpy())

with open('con_item2.npy', 'wb') as f:
