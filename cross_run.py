...
model.load_weights('./checkpoints/score_model2')
...
new_model.save_weights('./checkpoints/score_model_cross')
user_encoder = keras.Model(user_input, user_feat)
item_encoder = keras.Model(item_input, item_feat)
user_encoder.save_weights('./checkpoints/user_encoder_cross')
item_encoder.save_weights('./checkpoints/item_encoder_cross')

with open('user_final.npy', 'wb') as f:
    np.save(f, user_encoder(users).numpy())

with open('item_final.npy', 'wb') as f:
