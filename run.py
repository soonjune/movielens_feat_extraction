...
model.save_weights('./checkpoints/score_model')
user_encoder = keras.Model(user_input, user_feat)
item_encoder = keras.Model(item_input, item_feat)
user_encoder.save_weights('./checkpoints/user_encoder')
item_encoder.save_weights('./checkpoints/item_encoder')

with open('user.npy', 'wb') as f:
    np.save(f, user_encoder(users).numpy())

with open('item.npy', 'wb') as f:
