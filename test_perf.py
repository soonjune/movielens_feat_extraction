...
model.load_weights('./checkpoints/con_score_model_cross3')
outputs =  tf.keras.activations.sigmoid(outputs)

new_model = Model(inputs=[user_input, item_input], outputs=outputs)

new_model.compile(loss="binary_crossentropy", optimizer=tf.keras.optimizers.Adam(learning_rate=lr_schedule),
	metrics=["accuracy"])
...
# new_model.save_weights('./checkpoints/con_score_model_cross')
# user_encoder = keras.Model(user_input, user_feat)
# item_encoder = keras.Model(item_input, item_feat)
# user_encoder.save_weights('./checkpoints/con_user_encoder_cross')
# item_encoder.save_weights('./checkpoints/con_item_encoder_cross')

# with open('con_user_final.npy', 'wb') as f:
#     np.save(f, user_encoder(users).numpy())

# with open('con_item_final.npy', 'wb') as f:
